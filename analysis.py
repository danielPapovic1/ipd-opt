...
def _cohens_d(group1: np.ndarray, group2: np.ndarray) -> float:
    ...
    pooled_denom = ((n1 - 1) * var1 + (n2 - 1) * var2) / max(1, (n1 + n2 - 2))
    pooled_std = float(np.sqrt(max(pooled_denom, 1e-12)))
    return float((np.mean(group1) - np.mean(group2)) / pooled_std)


def statistical_significance_test(results_df: pd.DataFrame) -> Dict:
    """
    Perform one-way ANOVA and pairwise effect-size analysis on method comparison data.

    Expected columns: method, best_fitness, run
    Returns:
        {
            'anova_f': float,
            'anova_p': float,
    ...
    """
    required = {'method', 'best_fitness', 'run'}
    missing = required - set(results_df.columns)
    if missing:
        raise ValueError(f"Missing required columns for statistical test: {sorted(missing)}")

    method_groups = {
        method: group['best_fitness'].to_numpy(dtype=float)
        for method, group in results_df.groupby('method')
    }
    if len(method_groups) < 2:
        raise ValueError("At least two methods are required for ANOVA.")

    f_stat, p_value = f_oneway(*method_groups.values())

    effect_sizes: Dict[str, Dict[str, float]] = {}
    methods = sorted(method_groups.keys())
    for i, m1 in enumerate(methods):
        for m2 in methods[i + 1:]:
            g1, g2 = method_groups[m1], method_groups[m2]
            t_stat, t_p = ttest_ind(g1, g2, equal_var=False)
            effect_sizes[f"{m1}_vs_{m2}"] = {
...
def pareto_frontier_analysis(memory_df: pd.DataFrame) -> pd.DataFrame:
    """
    Identify Pareto-optimal (non-dominated) solutions maximizing fitness and minimizing bits.

    Expected columns: strategy_bits, best_fitness
    """
    required = {'strategy_bits', 'best_fitness'}
    missing = required - set(memory_df.columns)
    if missing:
        raise ValueError(f"Missing required columns for Pareto analysis: {sorted(missing)}")

    work_df = memory_df.copy().reset_index(drop=True)
    is_dominated = np.zeros(len(work_df), dtype=bool)

    for i in range(len(work_df)):
        bits_i = float(work_df.loc[i, 'strategy_bits'])
        fit_i = float(work_df.loc[i, 'best_fitness'])
        for j in range(len(work_df)):
            if i == j:
                continue
            bits_j = float(work_df.loc[j, 'strategy_bits'])
            fit_j = float(work_df.loc[j, 'best_fitness'])
            no_worse = bits_j <= bits_i and fit_j >= fit_i
            strictly_better = bits_j < bits_i or fit_j > fit_i
            if no_worse and strictly_better:
                is_dominated[i] = True
                break

    pareto_df = work_df.loc[~is_dominated].copy()
    pareto_df = pareto_df.sort_values(['strategy_bits', 'best_fitness'], ascending=[True, False])
