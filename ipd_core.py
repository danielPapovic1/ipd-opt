...
def create_strategy_from_bitstring(bitstring: str, memory_depth: int = 1) -> Strategy:
    ...
    assert len(bitstring) == expected_length, (
        f"Invalid bitstring length for memory_depth={memory_depth}: "
        f"expected {expected_length}, got {len(bitstring)}"
    )
    ...
    def bitstring_strategy(history: List[Tuple[Move, Move]]) -> Move:
        ...
        if memory_depth == 1:
            ...
            # Encode last `memory_depth` rounds as base-4 number (0..4^n-1)
            idx = 0
            start = max(0, len(history) - memory_depth)
            for i in range(start, len(history)):
                round_moves = history[i]
                digit = round_moves[0].value * 2 + round_moves[1].value
                idx = idx * 4 + digit
            # +1 because bit 0 is the initial move
            idx = idx + 1
