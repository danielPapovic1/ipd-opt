...
from experiments import (
    ExperimentRunner, run_full_experiment_suite,
    plot_pareto_frontier, plot_convergence_ci
)
from analysis import (
    ...
    statistical_significance_test, pareto_frontier_analysis
)
from zd_analysis import analyze_zd_properties
...
def run_full_experiments():
    ...
    # Ensure metadata captures runtime and seed policy.
    total_runtime_hours = (time.time() - start_time) / 3600.0
    results['total_runtime_hours'] = total_runtime_hours
    results['base_seed'] = int(results.get('base_seed', 42))
    results['seed_policy'] = results.get(
        'seed_policy',
        'base_seed + run index; method offsets x100 + [1..4]'
    )

    # 1) Statistical significance analysis
    method_df = results.get('method_comparison')
    if isinstance(method_df, pd.DataFrame):
        stats_out = statistical_significance_test(method_df)
        with open(os.path.join(output_dir, "statistical_analysis.json"), 'w', encoding='utf-8') as f:
            json.dump(stats_out, f, indent=2)

    # 2) Pareto frontier analysis
    memory_df = results.get('memory_depth')
    if isinstance(memory_df, pd.DataFrame):
        pareto_df = pareto_frontier_analysis(memory_df)
        pareto_df.to_csv(os.path.join(output_dir, "pareto_frontier.csv"), index=False)
        plot_pareto_frontier(memory_df, os.path.join(output_dir, "pareto_frontier.png"))

    # 3) ZD analysis on best evolved strategy from each method
    runner = ExperimentRunner(output_dir)
    evolved_best = results.get('evolved_best_by_method', {})
    if not evolved_best:
        runner.evolved_vs_reference(
            opponents=[TFT, ALLD, ALLC, TF2T, STFT, GRIM, PAVLOV],
            generations=500
        )
        evolved_best = runner.results_cache.get('evolved_best_by_method', {})
    zd_input = list(evolved_best.values())
    if zd_input:
        zd_results = analyze_zd_properties(zd_input)
        with open(os.path.join(output_dir, "zd_analysis.json"), 'w', encoding='utf-8') as f:
            json.dump(zd_results, f, indent=2)

    # 4) Convergence CI plot
    convergence_histories = results.get('convergence_histories', [])
    if convergence_histories:
        plot_convergence_ci(convergence_histories, os.path.join(output_dir, "convergence_ci.png"))

    # 5) Reproducibility metadata
    runner.log_reproducibility_metadata(
        results,
        os.path.join(output_dir, "reproducibility_metadata.json")
    )
...
2.3 Machine Learning
    ...
    - Training data: Random strategies evaluated in tournament (configurable memory depth, default=2)
""")
        ...
        try:
            ...
            if 'evolve_fitness' in memory.columns:
                memory = memory.drop(columns=['evolve_fitness'])
